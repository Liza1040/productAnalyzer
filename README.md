...
1. Для запуска frontend части перейти в `frontend\my-app` и запустить в терминале команду `npm start`
2. Для запуска backend части перейти в `backend\productAnalyzer` и запустить в терминале команду `python manage.py runserver`

## Часть 1: Backend (Django)
Парсер данных :
